 ...
 * <p>
 * The maximum number of roles the model may select is injected via the {@code pnyx.skill-panel.max-panel-size}
 * property; both the system and user prompts receive this as a {@code {{maxPanelSize}}} substitution.
...
public class AiSkillSelectorAdapter implements SkillSelectionSpi {
    ...
    private final int maxPanelSize;
    ...
    public AiSkillSelectorAdapter(ChatClient.Builder chatClientBuilder,
                                  SkillSelectionPromptProviderSpi prompts,
                                  ObjectMapper objectMapper,
                                  @Value("${pnyx.skill-panel.max-panel-size:4}") int maxPanelSize) {
        this.chatClientBuilder = chatClientBuilder;
        this.prompts = prompts;
        this.objectMapper = objectMapper;
        this.maxPanelSize = maxPanelSize;
    }
    ...
    public SkillSelectionResult selectSkills(String title, String problem,
        ...
        try {
            ...
            String maxSizeStr = String.valueOf(maxPanelSize);
            ...
            List<String> selected = ai.selectedRoles() != null
                ? ai.selectedRoles().stream().limit(maxPanelSize).toList()
                : List.of();
            ...
            log.info("Skill selection: selectedRoles={} (max={}), proposedNewSkills={}, rationale={}",
                selected, maxPanelSize, proposals.size(),
    ...
    private SkillSelectionResult fallback(List<SkillRoleDescriptor> availableRoles) {
        ...
        return new SkillSelectionResult(allIds, List.of(),
            "Fallback: using first " + maxPanelSize + " available roles");
    }
}
